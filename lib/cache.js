...
module.exports = class Cache {
  ...
  async loadCache({ allowPreReleases = false }) {
    const { latest, prerelease, refreshCache, isOutdated, lastUpdateStable } = this
    this.channel = allowPreReleases ? 'prerelease' : 'stable'

    if (!lastUpdateStable || isOutdated()) {
      await refreshCache()
    }
